...
describe('Room', () => {
  it('should render a room', () => {
    const wrapper = shallow(<Room title="Foo" capacity={4}/>);
    expect(wrapper.find('.Room-Title').text()).toEqual('Foo');
    expect(wrapper.find('.Room-Description').text()).toEqual('до 4 человек');
  });

  it('should change add _hover modifier depening on isHovered prop', () => {
    const wrapper = shallow(<Room isHovered={true} title="Foo" capacity={4}/>);
  ...
  it('should change add _pressed modifier depening on isPressed prop', () => {
    const wrapper = shallow(<Room isPressed={true} title="Foo" capacity={4}/>);
  ...
  it('should change add _disabled modifier depening on isPressed prop', () => {
    const wrapper = shallow(<Room isDisabled={true} title="Foo" capacity={4}/>);
